...
async def current_user_details(
    user: Annotated[
        user_schema.UserRequests, Depends(
            services.dependency_verify_cookie_ums_home)
    ],
):
    """
...
#!  It will give user's individual attendance summary


@router.post("/attendance/summary", status_code=status.HTTP_200_OK)
async def get_user_individual_attendance_route(
    user: Annotated[
        user_schema.UserRequests, Depends(
            services.dependency_verify_cookie_ums_home)
...
#!  It will give user's individual attendance details


@router.post("/attendance/detail", status_code=status.HTTP_200_OK)
async def get_user_individual_attendance_route(
    user: Annotated[
        user_schema.UserRequests, Depends(
            services.dependency_verify_cookie_ums_home)
